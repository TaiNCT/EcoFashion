...
export default function ApplySupplier() {
  ...
  const validateStep = (step: number): boolean => {
    ...
    switch (step) {
      case 0: // Basic Info
        if (!formData.supplierName.trim()) newErrors.supplierName = 'Tên công ty là bắt buộc';
      ...
      case 1: // Business Info
        if (!formData.bio.trim()) newErrors.bio = 'Mô tả về công ty là bắt buộc';
      ...
      case 3: // Identity Verification
        ...
        if (formData.identificationPictureFile.length === 0) {
          newErrors.identificationPictureFile = 'Vui lòng chọn ảnh CCCD/CMND' as any;
        }
        break;

      case 4: // Agreement
        if (!formData.agreedToTerms) newErrors.agreedToTerms = 'Bạn phải đồng ý với điều khoản' as any;
        break;
    }
  ...
  const updateSocialLinks = (platform: keyof FormData['socialLinks'], value: string) => {
    setFormData(prev => ({
      ...prev,
        ...
        return (
          <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
            ...
            <TextField
              ...
              helperText={errors.supplierName || `${formData.supplierName.length}/100 ký tự`}
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
            ...
            <Paper elevation={1} sx={{ p: 3 }}>
              ...
              <FileUpload
                ...
                onFilesChange={(files) => updateFormData('portfolioFiles', files)}
                accept="image/*"
                maxSize={5}
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
            ...
            <FormControl component="fieldset">
              ...
              <RadioGroup
                ...
                onChange={(e) => updateFormData('identificationType', e.target.value as 'cccd' | 'cmnd')}
                row
              >
                <FormControlLabel
                  value="cccd"
            ...
            <Paper elevation={1} sx={{ p: 3 }}>
              ...
              <FileUpload
                ...
                onFilesChange={(files) => updateFormData('identificationPictureFile', files)}
                accept="image/*"
                maxSize={5}
                required
        ...
        return (
          <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
            <Alert severity="success">
              Bạn đã hoàn thành các bước đăng ký! Vui lòng xem lại thông tin và xác nhận.
            </Alert>
            ...
            <Paper elevation={1} sx={{ p: 3 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, gap: 2 }}>
                  <Box sx={{ flex: 1 }}>
                    <Typography variant="body2" color="textSecondary">Tên Công ty:</Typography>
                ...
                <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, gap: 2 }}>
                  <Box sx={{ flex: 1 }}>
                    <Typography variant="body2" color="textSecondary">Loại hình:</Typography>
                    <Typography variant="body1">{formData.businessType}</Typography>
                  </Box>
                  <Box sx={{ flex: 1 }}>
                    <Typography variant="body2" color="textSecondary">Số {formData.identificationType.toUpperCase()}:</Typography>
                    <Typography variant="body1">{formData.identificationNumber}</Typography>
                  </Box>
                </Box>
              </Box>
            </Paper>
        ...
        <CardContent sx={{ p: 4 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'row', pt: 4 }}>
            ...
            {activeStep === steps.length - 1 ? (
              ...
              >
                {isSubmitting ? 'Đang xử lý...' : loading ? 'Đang gửi...' : 'Gửi đơn đăng ký'}
              </Button>
            ) : (
